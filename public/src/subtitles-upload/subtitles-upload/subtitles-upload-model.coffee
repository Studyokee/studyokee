define [
  ...
  SubtitlesUploadModel = Backbone.Model.extend(
    ...
    saveTranslation: (translatedSubtitles, callback) ->
      id = this.get('id')
      toLanguage = this.get('toLanguage')

      onSuccess = () =>
        subtitles = this.get('subtitles')
        subtitles.translation = translatedSubtitles
        this.trigger('change:subtitles')
      ...
      # TEMP HACK
      lyrics = []
      for line in translatedSubtitles
        lyrics.push(line.text)

      dataProvider.saveTranslation(id, toLanguage, lyrics, onSuccess)
